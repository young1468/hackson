...
function drawGame(t) {
  ...
  ctx.fillStyle = 'white';
  ctx.font = FONT_TEXT;
  const lines = splitText(event.text, 360);
  ...
  if (state.result) {
    drawCard(50, 780, 440, 90, '#ffd166');
    ctx.fillStyle = '#ffd166';
    ctx.fillText(state.result, 80, 835);
  }
}
...
function drawEnding() {
  drawTitle();
  drawCard(60, 240, 420, 320, state.profile.color);
  
  ctx.fillStyle = state.profile.color;
  ctx.font = '900 70px Orbitron';
  ctx.fillText(state.profile.avatar, 210, 340);
  ctx.fillStyle = 'white';
  ctx.font = '700 36px Rajdhani';
  ctx.fillText(state.ending.title, 130, 430);
  ctx.font = FONT_TEXT;
  ctx.fillText(state.ending.desc, 100, 500);
  
  drawButton(140, 760, 260, 70, '重新开始', state.profile.color);
  buttons = [{ id: 'restart', x: 140, y: 760, w: 260, h: 70 }];
}
...
function startGame() {
  ...
  // 核心修复点：将最大轮数强制限制在 4~6 轮，且绝不超过角色的可用事件数！
  const totalEvents = EVENTS[state.profile.id].length;
  state.maxSteps = Math.min(totalEvents, 4 + Math.floor(Math.random() * 3));
...
function choose(side) {
  ...
  if (s.crazy >= 100) { setTimeout(() => triggerEnding(CRISIS_ENDINGS.crazy), 1000); return; }
  if (s.money <= 0)   { setTimeout(() => triggerEnding(CRISIS_ENDINGS.money), 1000); return; }
  if (s.mood <= 0)    { setTimeout(() => triggerEnding(CRISIS_ENDINGS.mood), 1000); return; }
...
function finishGame() {
  ...
  const ending = roleEndings[endingIndex];
  state.ending = ending;
  addEndingToCollection(ending);
  state.scene    = 'ending';
  state.choosing = false;
...
canvas.addEventListener('pointerup', e => {
  ...
  if (btn.id === 'restart')      { state.scene = 'select'; }
